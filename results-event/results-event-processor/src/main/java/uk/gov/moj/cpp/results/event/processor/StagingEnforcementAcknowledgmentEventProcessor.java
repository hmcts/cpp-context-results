...
public class StagingEnforcementAcknowledgmentEventProcessor {
            ...
            if (isNotEmpty(enrichedDetails)) {
                ...
                for (EnrichedFineDetail item : enrichedDetails) {
                    ...
                    addIfNotNull(builder, Case.COURT_EMAIL,
                            ofNullable(ncesNotificationDetails).map(NcesNotificationDetails::email).orElse(null));
                    addIfNotNull(builder, Case.DIVISION,
                            ofNullable(ncesNotificationDetails).map(NcesNotificationDetails::division).orElse(null));
    ...
    private Stream<EnrichedFineDetail> enrichFineDetails(JsonObject def, JsonObject sourceSystem,
        ...
        List<JsonObject> fineAccountList = ofNullable(sourceSystem.getJsonArray(InactiveMigratedCase.DEFENDANT_FINE_ACCOUNT_NUMBERS))
                .stream()
    ...
    private DefendantDetails mapToDefendantDetails(JsonObject defendantJson) {
        ...
        final String defendantAddress = ofNullable(details.getJsonObject(PersonDetails.ADDRESS))
        ...
        final String originalDateOfConviction = ofNullable(defendantJson.getJsonArray(Offence.OFFENCES))
